<div>
            ...
            @else
                ...
                <button 
                    ...
                    class="flex items-center gap-1 px-3 py-1.5 border-2 rounded-lg text-sm font-medium transition
                        {{ $hasActiveFilters ? 'border-white bg-white/20 text-white' : 'border-white/30 bg-white/10 text-white hover:bg-white/20' }}">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    @if($showCreateModal)
        {{-- Backdrop escuro --}}
        <div class="fixed inset-0 bg-black/50 backdrop-blur-sm z-40 flex items-center justify-center p-4" wire:click="closeCreateModal"></div>
    ...
    @if($showMergeModal)
        {{-- Backdrop escuro --}}
        <div class="fixed inset-0 bg-black/50 backdrop-blur-sm z-40" wire:click="closeMergeModal"></div>
        ...
        <div class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[95%] max-w-md bg-white rounded-2xl shadow-2xl border-2 border-gray-300 z-50">
            ...
            <div class="p-4">
                <div class="space-y-4">
                    ...
                    <div class="space-y-2">
                        @php
                            $filteredTables = $tables->whereIn('id', $selectedTables);
                        @endphp
                        ...
                        @foreach($filteredTables as $table)
                            <label class="flex items-center p-3 border-2 rounded-lg cursor-pointer transition {{ $mergeDestinationTableId == $table->id ? 'bg-orange-50 border-orange-500 ring-2 ring-orange-200' : 'border-gray-300 hover:border-orange-400' }}">
                                ...
                                <span class="ml-auto text-sm">
                                    @if(isset($table->checkId) && $table->checkId)
