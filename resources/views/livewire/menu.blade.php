<div>
    ...
    {{-- Categorias --}}
    <div class="px-4 py-3 bg-gray-50 border-b sticky top-28 z-20 overflow-x-auto">
        <div class="flex gap-2">
            <button 
                wire:click="selectCategory(null)"
                class="px-4 py-2 rounded-full whitespace-nowrap text-sm font-medium transition shadow-sm
                    {{ !$selectedCategoryId ? 'bg-orange-500 text-white' : 'bg-white text-gray-700 hover:bg-gray-100 border border-gray-300' }}">
                Todos
            </button>
            @foreach($categories as $category)
                <button 
                    ...
                    class="px-4 py-2 rounded-full whitespace-nowrap text-sm font-medium transition shadow-sm
                        {{ $selectedCategoryId == $category->id ? 'bg-orange-500 text-white' : 'bg-white text-gray-700 hover:bg-gray-100 border border-gray-300' }}">
                    {{ $category->name }}
                </button>
            @endforeach
        </div>
