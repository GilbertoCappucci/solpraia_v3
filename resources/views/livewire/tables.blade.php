<div wire:poll.{{ $pollingInterval }}s>
    ...
    @if($showMergeModal)
        <livewire:merge-tables :selected-tables="$selectedTables" :tables="$tables" wire:key="merge-tables" />
    @endif
</div>
    ...
    (function(){
        function forwardEvent(name){
            window.addEventListener(name, function(e){
                const payload = e && e.detail ? e.detail : undefined;
                // Prefer Livewire global object
                const lw = window.Livewire || window.livewire || window.livewire_app || null;
                try{
                    if(lw && typeof lw.emit === 'function'){
                        lw.emit(name, payload);
                    } else if(window.dispatchEvent){
                        // fallback: emit a custom event on document that Livewire may pick
                        document.dispatchEvent(new CustomEvent(name, { detail: payload }));
                    }
                }catch(err){
                    console.error('Error forwarding Livewire event', name, err);
                }
            });
        }

        forwardEvent('merge-completed');
        forwardEvent('merge-closed');
    })();
</script>
